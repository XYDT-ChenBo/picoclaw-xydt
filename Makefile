...
build: generate
	...
	@mkdir -p $(BUILD_DIR)
	# GOOS=$(PLATFORM) GOARCH=$(ARCH) GOARM=7 $(GO) build $(GOFLAGS) $(LDFLAGS) -o $(BINARY_PATH) ./$(CMD_DIR)
	GOOS=$(PLATFORM) GOARCH=$(ARCH) $(GO) build $(GOFLAGS) $(LDFLAGS) -o $(BINARY_PATH) ./$(CMD_DIR)
