...
// Operator method names (openclaw gateway protocol).
const (
	MethodAgent      = "agent"
...
// Server is a WebSocket server that accepts openclaw node and operator connections.
// It speaks the openclaw gateway protocol (version 3). Node role: headless Linux,
// iOS, Android, macOS. Operator role: control-plane clients (chat, config, node.invoke).
type Server struct {
	cfg              ServerConfig
	registry         *Registry
	operatorRegistry *OperatorRegistry
	operatorBackend  OperatorBackend // nil = reject operator connections
...
// operatorBackend may be nil; if so, operator role connections are rejected.
func NewServer(cfg ServerConfig, registry *Registry, operatorBackend OperatorBackend) *Server {
	opReg := (*OperatorRegistry)(nil)
	if operatorBackend != nil {
		opReg = NewOperatorRegistry()
	}
	return &Server{
		cfg:              cfg,
		registry:         registry,
...
func (s *Server) handleConnection(conn *websocket.Conn, remoteIP string) {
	...
	// Step 4: Validate role and branch.
	// Token validation is skipped for minimum viable (can be added later).
	switch params.Role {
	case "node":
		// Node flow continues below.
	case "operator":
		if s.operatorBackend == nil || s.operatorRegistry == nil {
			msg := "operator connections not supported (no backend configured)"
			log("debug", "operator rejected", map[string]any{"reason": "no backend"})
			sendRes(frameID, false, nil, &ErrBody{Code: ErrCodeInvalidRequest, Message: msg})
			return
		}
		s.handleOperatorConnection(conn, connID, remoteIP, &params, nonce, frameID, sendRes, log)
		return
	default:
		msg := fmt.Sprintf("unsupported role: %q (use 'node' or 'operator')", params.Role)
		log("debug", "invalid role", map[string]any{"role": params.Role})
		sendRes(frameID, false, nil, &ErrBody{Code: ErrCodeInvalidRequest, Message: msg})
		return
	}

	// Step 5: Validate nonce for node (optional, skip if no device).
...
// handleOperatorConnection runs the full lifecycle of an operator WebSocket connection.
func (s *Server) handleOperatorConnection(
	conn *websocket.Conn,
	connID, remoteIP string,
	params *ConnectParams,
	nonce, frameID string,
	sendRes func(id string, ok bool, payload any, errBody *ErrBody),
	log func(level, msg string, fields map[string]any),
) {
	session := s.operatorRegistry.Register(connID, conn, params)
	defer s.operatorRegistry.Unregister(connID)

	hostname, _ := os.Hostname()
	operatorMethods := []string{
		MethodAgent, MethodChatSend, MethodNodeList, MethodNodeInvoke, MethodHealth,
	}
	helloOk := HelloOkPayload{
		Type:     "hello-ok",
		Protocol: ProtocolVersion,
		...
		Features: HelloOkFeatures{
			...
			Events:  []string{EventConnectChallenge, "tick"},
		},
		Policy: HelloOkPolicy{
			MaxPayload:       MaxPayloadBytes,
			MaxBufferedBytes: MaxBufferedBytes,
	...
	log("info", "operator connected", map[string]any{
		"client_id": params.Client.ID,
		"scopes":    params.Scopes,
	})

	// Tick keepalive (operator clients may expect it).
	tickDone := make(chan struct{})
	go func() {
		ticker := time.NewTicker(TickIntervalMs * time.Millisecond)
		defer ticker.Stop()
	...
	for {
		...
		if err != nil {
			if !websocket.IsCloseError(err, websocket.CloseNormalClosure, websocket.CloseGoingAway) {
				log("debug", "operator read error", map[string]any{"error": err.Error()})
			}
			return
		}
		...
		s.handleOperatorRequest(reqMethod, reqID, paramsRaw, session, sendRes, log)
	}
}

// handleOperatorRequest dispatches an operator method request.
func (s *Server) handleOperatorRequest(
	method, reqID string,
	...
	switch method {
	case MethodAgent, MethodChatSend:
		s.handleOperatorAgent(ctx, reqID, params, session, sendRes, log)
	case MethodNodeList:
		s.handleOperatorNodeList(reqID, sendRes)
	case MethodNodeInvoke:
		s.handleOperatorNodeInvoke(ctx, reqID, params, sendRes, log)
	case MethodHealth:
		s.handleOperatorHealth(reqID, sendRes)
	default:
		log("debug", "operator method ignored", map[string]any{"method": method})
		sendRes(reqID, true, map[string]any{"ok": true}, nil)
	}
}

func (s *Server) handleOperatorAgent(
	ctx context.Context,
	reqID string,
	params map[string]any,
	_ *OperatorSession,
	...
	if message == "" {
		sendRes(reqID, false, nil, &ErrBody{Code: ErrCodeInvalidRequest, Message: "message required"})
		return
	}
	response, err := s.operatorBackend.ProcessMessage(ctx, message, sessionKey)
	if err != nil {
		log("warn", "agent error", map[string]any{"error": err.Error()})
		sendRes(reqID, false, nil, &ErrBody{Code: ErrCodeUnavailable, Message: err.Error()})
		return
	}
	sendRes(reqID, true, map[string]any{
		"runId":    uuid.New().String(),
		"response": response,
	}, nil)
}

func (s *Server) handleOperatorNodeList(reqID string, sendRes func(id string, ok bool, payload any, errBody *ErrBody)) {
	connected := s.registry.ListConnected()
	nodes := make([]map[string]any, 0, len(connected))
	...
	sendRes(reqID, true, map[string]any{"ts": time.Now().UnixMilli(), "nodes": nodes}, nil)
}

func (s *Server) handleOperatorNodeInvoke(
	ctx context.Context,
	reqID string,
	params map[string]any,
	sendRes func(id string, ok bool, payload any, errBody *ErrBody),
	...
	if nodeID == "" || command == "" {
		sendRes(reqID, false, nil, &ErrBody{Code: ErrCodeInvalidRequest, Message: "nodeId and command required"})
		return
	}
	timeoutMs := 30_000
	...
	var p any
	if pr, ok := params["params"]; ok && pr != nil {
		p = pr
	}
	result := s.registry.Invoke(nodeID, command, p, timeoutMs)
	if !result.Ok {
		errMsg := "node not connected"
		if result.Error != nil && result.Error.Message != "" {
			errMsg = result.Error.Message
		}
		sendRes(reqID, false, nil, &ErrBody{Code: ErrCodeUnavailable, Message: errMsg})
		return
	}
	payload := result.Payload
	if result.PayloadJSON != nil {
		var decoded any
		if err := json.Unmarshal([]byte(*result.PayloadJSON), &decoded); err == nil {
			payload = decoded
		}
	}
	sendRes(reqID, true, map[string]any{
...
func (s *Server) handleOperatorHealth(reqID string, sendRes func(id string, ok bool, payload any, errBody *ErrBody)) {
	sendRes(reqID, true, map[string]any{"ok": true}, nil)
}
